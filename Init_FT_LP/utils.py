...
def validata_dataset_params(cfg):
    if cfg.dataset.name == 'cub':
        ...
        if cfg.dataset.num_classes != 100:
            cfg.dataset.num_classes = 100
    elif cfg.dataset.name == 'imagenet-r':
        if cfg.dataset.num_classes != 200:
            cfg.dataset.num_classes = 200
    else:
...
def shrink_cov(cov, device, alpha=1):
    iden = torch.eye(cov.shape[0]).to(device)
    cov_ = cov + (alpha*iden)
    return cov_
...
def build_data_provider(local_batch_size, num_clients, dataset, num_classes, alpha, num_client_samples, drop_last: bool = False):
    ...
    if dataset == 'cub':
        ...
        train_dataset = StanfordCars(data_dir, split="train", transform=train_transform, download=False)
        test_dataset = StanfordCars(data_dir, split="test", transform=test_transform, download=False)
        if num_client_samples is not None:
...
class FLModel(IFLModel):
    ...
    def fl_forward(self, batch, user_labels = None) -> FLBatchMetrics:
        ...
        output = self.model(features) #/ self.temp
...
class MetricsReporter(FLMetricsReporter):
    ...
    def compare_metrics(self, eval_metrics, best_metrics):
        ...
        if self.wandb_dict['activate']:
            if self.round1 == 1:
                wandb.log({f"Current eval accuracy": best_metrics})
                self.round1 += 1
    ...
    for arg in vars(args):
        if getattr(args, arg) is not None:
            if arg == 'wandb':
                ...
                if args.fedcof == 1:
                    cfg.trainer.fedcof = True
                else:
                    cfg.trainer.fedcof = False
            elif arg == 'fed3r':
                if args.fed3r == 1:
                    cfg.trainer.fed3r = True
                else:
                    cfg.trainer.fed3r = False
            elif arg == 'need_cov':
                if args.need_cov == 1:
                    cfg.trainer.need_cov = True
                else:
                    cfg.trainer.need_cov = False
            elif arg == 'subsets':
                cfg.trainer.subsets = args.subsets
            elif arg == 'shrink':
                cfg.trainer.shrink = args.shrink
            elif arg == 'algorithm':
