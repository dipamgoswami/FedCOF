...
class SyncTrainer(FLTrainer):
    ...
    def cp_init(self, num_classes, device, model, users_per_round, model_type, need_cov, fed3r, num_subsets, feat_dims, data_provider: IFLDataProvider):
        num_users_on_worker = data_provider.num_train_users()
        users_per_round_on_worker = users_per_round
        ...
        # FedFeCAM Initialization
        clients_init = self._client_selection(
                    num_users_on_worker,
                    num_users_on_worker,
                    data_provider,
                    model,
        ...
        clients_init_ = []
        seqq = []
        for cln, client in enumerate(clients_init):
            seqq.append(int(client.name[7:]))
        # print(seqq)
        seqq.sort()
        clients_seq = random.sample(seqq, users_per_round_on_worker)
        # print(clients_seq)
        for cln, client in enumerate(clients_init):
            if int(client.name[7:]) in clients_seq:
                clients_init_.append(client)

        cov_mats = {new_list: [] for new_list in range(users_per_round_on_worker)}
        if num_subsets > 1:
            mu = {new_list: [] for new_list in range(users_per_round_on_worker*num_subsets)}
            count = {new_list: [] for new_list in range(users_per_round_on_worker*num_subsets)}
        else:
            mu = {new_list: [] for new_list in range(users_per_round_on_worker)}
            count = {new_list: [] for new_list in range(users_per_round_on_worker)}

        with torch.no_grad():
            cc = 0
            gm = 0  # gram matrices for Fed3R
            # all_feats = []
            for cln, client in enumerate(clients_init_):    # self.client_models().items()):
                if client.name in self.sampled_clients:
                    mu[cln] = {new_list: [] for new_list in range(num_classes)}
                    
                    continue
                else:
                    self.sampled_clients.append(client.name)
                    # model.fl_get_module().eval()
                    cln_feats = []
                    cln_count = 0
                    feats = {new_list: [] for new_list in range(num_classes)}
                    covs = {new_list: [] for new_list in range(num_classes)}
                    if num_subsets > 1:
                        sample_means = {sub: {new_list: [] for new_list in range(num_classes)} for sub in range(num_subsets-1)}
                        sample_cls_counts = {sub: {new_list: [] for new_list in range(num_classes)} for sub in range(num_subsets-1)}
                    means = {new_list: [] for new_list in range(num_classes)}
                    cls_counts = {new_list: 0 for new_list in range(num_classes)}
                    ...
                    for batch in client.dataset.train_data():
                        ...
                        if 'resnet' in model_type:
                            embs = torch.nn.Sequential(*list(model.children())[:-1])(features)
                        elif 'vit' in model_type:
                            embs = model.forward_features(features)[:,0]
                        else:
                            embs = model.features(features)
                            embs = F.adaptive_avg_pool2d(embs, 1)
                        cc += len(batch["labels"])
                        if not 'vit' in model_type:
                            embs = embs.squeeze(2).squeeze(2)

                        if fed3r:
                            cln_feats.append(embs)
                            cln_count += len(features)

                        for label_idx in range(num_classes):
                            mask = batch_label == label_idx
                            embs_ = embs[[mask]]
                            if mask.float().sum() > 0:
                                feats[label_idx].append(embs_)

                    for label_idx in range(num_classes):
                        if len(feats[label_idx]) == 1:
                            x = feats[label_idx][0]
                            feats[label_idx].append(x+(0.1**0.5)*torch.randn(x.shape).to(device))
                        if len(feats[label_idx]) > 1:  # covariance from a single vector gives Nan values
                            feats[label_idx] = torch.cat(feats[label_idx],0)
                            
                            # Randomly sample g sets of features to compute g means and class counts
                            if num_subsets > 1 and len(feats[label_idx]) >= 4:
                                num_subsets_ = min(num_subsets, math.floor(len(feats[label_idx])/2))
                                sub_c = math.floor(len(feats[label_idx])/num_subsets_)
                                ind = torch.randperm(len(feats[label_idx]))  #.to(dtype=torch.long, device=device)
                                cls_feats = feats[label_idx][ind]
                                for sub in range(num_subsets_):
                                    if sub == num_subsets_-1:
                                        fts = cls_feats
                                    else:
                                        fts = cls_feats[:sub_c]
                                        cls_feats = cls_feats[sub_c:]
                                    if sub == 0:
                                        cls_counts[label_idx] = len(fts)
                                        means[label_idx] = fts.mean(0)
                                    else:
                                        sample_cls_counts[sub-1][label_idx] = len(fts)
                                        sample_means[sub-1][label_idx] = fts.mean(0)            
                            else:
                                means[label_idx] = feats[label_idx].mean(0)
                                
                                cls_counts[label_idx] = len(feats[label_idx])
                                if need_cov and not fed3r:
                                    covar = torch.cov(feats[label_idx].T)
                                    if diag:
                                        covar = torch.diag(torch.diagonal(covar,0))
                                    if spherical:
                                        sigma = torch.diagonal(covar).mean()
                                        covar = torch.diag(sigma.repeat(covar.shape[0]))
                                    covs[label_idx] = shrink_cov(covar, device)

                    if fed3r:
                        cln_feats = torch.cat(cln_feats,0)
                        gm_ = torch.matmul(cln_feats.T, cln_feats)  # gram matrix
                        gm += gm_
                    else:
                        if need_cov:
                            cov_mats[cln] = covs
                    ...
                    count[cln] = cls_counts
                    mu[cln] = means

                    if num_subsets >1:
                        for ij in range(1,num_subsets):
                            count[cln + (ij*users_per_round_on_worker)] = sample_cls_counts[ij-1]
                            mu[cln + (ij*users_per_round_on_worker)] = sample_means[ij-1]

        if fed3r:
            return mu, gm, count
        else:
            return mu, cov_mats, count
    ...
    def train(
        ...
        """
        #print(f'ln 184 In train method...')
        ...
        for epoch in range(1, num_int_epochs + 1):
            print(f'Global epoch {epoch}')
            # rounds is users / users per round
            for round in range(1, num_rounds_in_epoch + 1):
                print(f'Round {round}')
            # for round in tqdm(
            ...
            # ):
                ...
                # agg metric clients are currently training clients. this is governed by the
                # self.cfg.use_train_clients_for_aggergation_metrics flag
                ...
                #### Evaluation phase ####
                if rank == 0:
                    #print(f'ln 283 evaluation phase')
            ...
            if self.stop_fl_training(
                ...
                break
            '''if epoch in [0, 2]:
                cp = self.global_model().model.state_dict()
                torch.save(cp, f'model_checkpoints/round{epoch}checkpoint.pt')'''
    ...
    def _client_selection(
        ...
        # these are a list of training ds clients
        clients_triggered = [
